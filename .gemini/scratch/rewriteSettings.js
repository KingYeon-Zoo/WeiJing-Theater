...
if (!scriptMatch) {
  console.error("Could not find <script setup> block");
  process.exit(1);
}
                  ...
                  <div class="accordion-foot">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <button class="btn btn-primary" :disabled="agentSaving" @click="saveAgentCfg(a.type)">
                        <Loader2 v-if="agentSaving" :size="14" class="animate-spin" />
                        Save Agent
                      </button>
                    </div>
                  </div>
        ...
        <div v-else-if="tab === 'skills'" class="settings-panel fade-in skill-panel-layout">
          <aside class="skill-nav">
            ...
            <button
              ...
              :key="a.type"
              :class="['skill-nav-btn', { active: selectedAgent === a.type }]"
          ...
          <main class="skill-workspace">
            ...
            <div v-if="!currentSkills.length" class="config-empty border-dashed">
              <FileText :size="24" class="dim mb-2" />
              <div>No skills installed for this agent.</div>
            </div>

            <div class="agent-list" p-0>
              <div v-for="s in currentSkills" :key="s.id" class="agent-accordion" :class="{'is-open': editingSkill === s.id}">
                ...
                <div v-if="editingSkill === s.id" class="accordion-body">
                  <textarea
                    ...
                    class="textarea mono"
                    rows="16"
                    placeholder="Write SKILL.md contents..."
                  />
                  <div class="accordion-foot">
                    ...
                    <div class="flex items-center gap-2">
                       ...
                       <button class="btn btn-primary" :disabled="skillSaving" @click="saveSkill(s.id)">
                         <Loader2 v-if="skillSaving" :size="14" class="animate-spin" />
                         Save Skill
                       </button>
                    </div>
                  </div>
      ...
      <form class="modal figma-modal" @submit.prevent="applyWeijingPreset">
        ...
        <div class="modal-body field-stack">
          <label class="field">
            ...
            <span class="field-hint">No account? <a href="https://api.chatfire.site/" target="_blank" rel="noopener">Register →</a></span>
          </label>
        </div>
        <div class="modal-footer">
...
.settings-brand-logo { width: 32px; height: 32px; filter: invert(); }
...
.section-title {
  font-size: 18px; font-weight: 600; letter-spacing: -0.14px;
}
...
.config-row.is-active {
  background: linear-gradient(90deg, rgba(8, 255, 8, 0.03), transparent 200px);
}
...
.agent-icon {
  ...
  background: var(--bg-2); display: flex; align-items: center; justify-content: center; font-size: 18px;
}
.agent-icon-sm {
   width: 24px; height: 24px; border-radius: 4px; background: var(--bg-2); display: flex; align-items: center; justify-content: center; font-size: 12px;
}
...
.skill-panel-layout {
  display: flex;
  margin: -40px; height: calc(100% + 80px);
}
.skill-nav { width: 200px; border-right: 1px solid var(--border); background: var(--bg-1); display: flex; flex-direction: column; padding: 20px 10px; gap: 8px; }
.skill-nav-title { padding-left: 10px; color: var(--text-3); margin-bottom: 8px; }
.skill-nav-btn {
  display: flex; align-items: center; gap: 8px; padding: 8px 10px; border-radius: var(--radius-md); cursor: pointer; border: none; background: transparent; transition: 0.15s; font-size: 13px; font-weight: 500;
}
...
@media (max-width: 800px) {
  ...
  .settings-container { border-radius: 0; min-height: 100vh; }
  .preset-grid { grid-template-columns: 1fr; }
  .config-row { flex-direction: column; align-items: flex-start; }
  .skill-panel-layout { flex-direction: column; height: auto; }
  .skill-nav { width: 100%; flex-direction: row; overflow-x: auto; padding: 10px; }
}
</style>
`;
fs.writeFileSync(path, newTemplate);
console.log("Settings.vue rewritten successfully.");
