...
export const errorHandler: MiddlewareHandler = async (c, next) => {
  try {
    ...
    if (status >= 500) {
      ...
      console.error(`${colors.yellow}[WARN]${colors.reset} ${c.req.method} ${c.req.path} - ${err.message}`)
    }
    return c.json({ code: status, message: err.message || 'Internal Server Error' }, status as any)
  }
}
