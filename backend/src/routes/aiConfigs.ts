...
const WEIJING_AGENT_DEFAULTS = [
  ...
  { agentType: 'creative_interviewer', name: '创作访谈' },
] as const

const WEIJING_AGENT_MODEL = 'gemini-3-pro-preview'
...
function buildProbe(serviceType: string, provider: string, baseUrl: string, model?: string, apiKey?: string) {
  ...
  if (p === 'ali') {
    return {
      method: 'POST',
      url: joinProviderUrl(baseUrl, '/api/v1', '/services/aigc/image-generation/generation'),
  ...
  if (p === 'volcengine') {
    return {
      method: 'POST',
      url: joinProviderUrl(baseUrl, '/api/v3', '/images/generations'),
      headers: bearerHeaders(apiKey, true),
      body: {},
    }
  }
...
// POST /ai-configs/weijing-preset
app.post('/weijing-preset', async (c) => {
  const body = await c.req.json()
  ...
  for (const preset of WEIJING_PRESET_SERVICES) {
  ...
  logTaskSuccess('AIConfig', 'weijing-preset-applied', {
    serviceCount: WEIJING_PRESET_SERVICES.length,
    agentCount: WEIJING_AGENT_DEFAULTS.length,
  })

  return success(c, {
    configs,
    agents,
    agent_model: WEIJING_AGENT_MODEL,
  })
})
