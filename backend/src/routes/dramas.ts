...
function getDefaultEpisodeConfigIds() {
  ...
  return {
    imageConfigId: pick('image'),
    audioConfigId: pick('audio'),
  }
}

function ensureDramaEpisodeConfigs(dramaId: number) {
  const { imageConfigId, audioConfigId } = getDefaultEpisodeConfigIds()
  if (!imageConfigId && !audioConfigId) return
  ...
  for (const episode of episodes) {
    ...
    if (!episode.imageConfigId && imageConfigId) updates.imageConfigId = imageConfigId
    if (!episode.audioConfigId && audioConfigId) updates.audioConfigId = audioConfigId
